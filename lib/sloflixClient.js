...
class SloFlixClient {
  ...
  /**
   * Best-effort liveness check for a resolved CDN stream URL: fires the same
   * request shape the /play/ proxy will (Referer/Origin the SloFlix CDN
   * expects, a tiny Range request so we don't pull the whole file) and treats
   * anything other than a real HTTP error / timeout as "alive". Used to drop
   * dead sources (expired links, downed mirrors) from what Stremio/Nuvio see,
   * instead of listing several "Stream" entries where only one actually plays.
   */
  static async probeStream(streamUrl, { timeoutMs = 6000 } = {}) {
    ...
    try {
      ...
      // We only care whether the source responds/serves data, not the bytes
      // themselves - cancel the body immediately so we don't wait on it.
  ...
  async resolveStream(mediaId) {
    ...
    for (const sourceObj of sources) {
      ...
      candidates.push({ streamUrl, subtitleUrl, score, label: SloFlixClient.labelForScore(score) });
    }
